    ...
    public function editmodal($name,$id,Request $request)
    {
        switch ($name) {
            case 'user':
                ...
                $option='';
                $option2='';
                $model = DataUser::find(explode('&',Crypt::decryptstring($id))[0]);
                $keys=0;
                foreach (explode(',',env('tabel_sumber')) as $key => $value) {
                    if($model->tbl_sumber == $value){
                        $keys = $key;
                    }
                    $option .='<option value="'.$key.'"'.($model->tbl_sumber == $value ? "selected" :'').'>'.$value.'</option>';
                }
                foreach (DataRole::where('nama_app',env('nama_app'))->where('usr_akses','!=','superuser')->get() as $key => $value) {
                    $option2 .='<option value="'.$value->id_uaks.'"'.($model->id_uaks == $value->id_uaks ? "selected" :'').'>'.$value->ket_akses.'</option>';
                }
                $tabel_sumber = explode(',',env('tabel_sumber'))[($keys)];
                $kolom_nama = explode(',',env('kolom_nama'))[($keys)];
                $kolom_id = explode(',',env('kolom_id'))[($keys)];
                $option3='<option value="" disabled selected>Nama Lengkap </option>';
                foreach (\DB::table($tabel_sumber)->get() as $key => $value) {
                    $option3 .='<option value="'.Crypt::encryptString($value->$kolom_id).'"'.($model->key_relasi == $value->$kolom_id ? "selected" :'').'>'.$value->$kolom_nama.'</option>';
                ...
                $form = '<div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>Nama User</label>
                                    <input type="text" name="namauser" class="form-control" value="'.$model->namauser.'" placeholder="Nama User" maxlength="50" required="required">
                                    <span class="help-block has-error namauser_error"></span>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>Nama Lengkap</label>
                                    <select name="nama_lkp" id="nama_lkp" style="width: 100%;" class="form-control" required="required">'.$option3.'</select>
                                </div>
                            </div>
                        </div>';
                $footer = '<button type="submit" id="btn-submit" class="btn btn-default btn-primary pull-right" data-ref="PUT" data-type="user"><i class="fa fa-check"></i> Simpan</span></button>';
                return response()->json(['form' => $form,'title' => $title, 'size' => $size, 'footer'=>$footer]);
            break;
            case 'role':
